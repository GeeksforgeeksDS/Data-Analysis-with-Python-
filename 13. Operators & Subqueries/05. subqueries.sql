...
select * from restaurants where rating_count = (select min(rating_count) as 'rating_count' from restaurants) and city = "Abohar";
...
select count(*) as "total_restaurants" from restaurants where rating >= (select avg(rating) from restaurants);

-- 4. Which restaurant of Delhi has generated most revenue
select max(rating_count * cost) from restaurants;
select * from restaurants where rating_count * cost = 5000000;
select * from restaurants where rating_count*cost = (select max(rating_count*cost) from restaurants) and city = "Delhi";
select * from restaurants where rating_count * cost = (select max(rating_count * cost) from restaurants where city = 'Delhi');
select * from restaurants where rating_count * cost = (select max(rating_count * cost) from restaurants where city = 'Delhi') and city = 'Delhi';
...
select city , avg(rating_count) from restaurants group by city order by avg(rating_count) desc limit 5;
